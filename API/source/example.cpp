...
int main() {
  std::shared_ptr<drawNS::Draw3DAPI> api(new APIGnuPlot3D(-5,5,-5,5,-5,5,20)); //włacza gnuplota, pojawia się scena [-5,5] x [-5,5] x [-5,5] odświeżana co 1000 ms
  ...
  api->draw_polygonal_chain(vector<Point3D> {drawNS::Point3D(0,0,0),
	...
	drawNS::Point3D(4,0,0),drawNS::Point3D(4,0,4)},"purple"); //rysuje fioletową łamaną  cout << "pojawiła się fioletowa łamana" << endl;
  
  wait4key();
  api->change_ref_time_ms(20);
  ...
  wait4key();
  api->change_ref_time_ms(20);//odświerzanie sceny zmienione na opcję "tylko za pomocą metody redraw()"
